...
public class CalendarStampServiceImpl implements CalendarStampService {
    ...
    @Override
    public Map<LocalDate, List<CalendarStampResponseDTO>> getAllStampsWhenYearAndMonth(
        Long memberId, Integer year,
        Integer month) {
        if (year == null || month == null) {
            throw new YearOrMonthException(String.format(YEAR_OR_MONTH_ERROR, year, month));
        }

        List<CalendarStamp> findList = calendarStampRepository.getAllStampsWhenYearAndMonth(
            memberId, year, month);
