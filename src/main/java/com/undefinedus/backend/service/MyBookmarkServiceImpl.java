...
public class MyBookmarkServiceImpl implements MyBookmarkService {
    ...
    public void insertBookMark(Long memberId, BookmarkRequestDTO requestDTO) {
        ...
        AladinBook findAladinBook = aladinBookRepository.findByTitle(requestDTO.getTitle())
            .orElseThrow(() -> new BookNotFoundException(
                String.format(AladinBook_NOT_FOUND, requestDTO.getTitle())));
    ...
    public ScrollResponseDTO<MyBookmarkResponseDTO> getMyBookmarkList(Long targetMemberId,
        ...
        Long lastId = myBookmarks.isEmpty() ?
            ...
            myBookmarks.get(myBookmarks.size() - 1)
    ...
    @Override
    public void updateMyBookmark(Long memberId, Long bookmarkId,
        MyBookmarkUpdateRequestDTO requestDTO) {
        ...
        MyBookmark findMyBookmark = myBookmarkRepository.findByIdAndMemberId(bookmarkId, memberId)
            .orElseThrow(() -> new BookmarkNotFoundException(
                String.format(BOOKMARK_NOT_FOUND, memberId, bookmarkId)));
    ...
    public void insertOtherMemberBookmarkToMe(Long loginMemberId, Long targetBookmarkId) {
        ...
        MyBookmark findTargetBookmark = myBookmarkRepository.findById(targetBookmarkId)
            .orElseThrow(() -> new BookmarkNotFoundException(
                String.format(BOOKMARK_NOT_FOUND_BY_ID, targetBookmarkId)));

        Member loginMember = memberRepository.findById(loginMemberId)
            .orElseThrow(
                () -> new MemberNotFoundException(String.format(USER_NOT_FOUND, loginMemberId)));
